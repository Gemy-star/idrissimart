...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--send-sms",
            action="store_true",
            help="Send SMS alert to admins if payments are marked as failed.",
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if count > 0:
            if dry_run:
                ...
                if send_sms and SMSService.is_enabled() and failed_count > 0:
                    try:
                        admin_phone = getattr(config, 'ADMIN_ALERT_PHONE', None)
                        total_amount = sum(payment.amount for payment in stale_payments)

                        if admin_phone:
                            sms_message = f"{config.SITE_NAME}: {failed_count} مدفوعات متعثرة تم إلغاؤها ({total_amount:.2f})"
