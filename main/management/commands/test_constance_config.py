...
from main.services import TwilioService
...
class Command(BaseCommand):
    ...
    def test_paypal_config(self):
        ...
        try:
            ...
            if client_id and client_secret:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'  ✅ PayPal Client Secret: {client_secret[:20]}...')
                )
                self.stdout.write(
                    self.style.SUCCESS(f'  ✅ PayPal Mode: {mode}')
                )
                ...
                if paypal_service.client_id and paypal_service.client_secret:
                    self.stdout.write(
                        self.style.SUCCESS('  ✅ PayPal service initialized successfully')
                    )
                else:
                    self.stdout.write(
    ...
    def test_paymob_config(self):
        ...
        try:
            ...
            from main.payment_services import PaymobService
            paymob_service = PaymobService()
            if paymob_service.api_key:
                self.stdout.write(
                    self.style.SUCCESS('  ✅ Paymob service initialized successfully')
                )
            else:
                self.stdout.write(
    ...
    def test_twilio_config(self):
        ...
        try:
            ...
            if account_sid and auth_token:
                self.stdout.write(
                    self.style.SUCCESS(f'  ✅ Twilio Account SID: {account_sid[:20]}...')
                )
                self.stdout.write(
                ...
                if twilio_service.client:
                    self.stdout.write(
                        self.style.SUCCESS('  ✅ Twilio client initialized successfully')
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR('  ❌ Twilio client initialization failed')
                    )
            else:
                self.stdout.write(
            ...
            self.stdout.write(
                self.style.SUCCESS(f'  ✅ Mobile Verification Enabled: {enable_verification}')
            )
            self.stdout.write(
    ...
    def test_payment_service(self):
        ...
        try:
            ...
            if supported_providers:
                self.stdout.write(
                    self.style.SUCCESS(f'  ✅ Found {len(supported_providers)} supported payment providers:')
                )
                for provider in supported_providers:
                    mode_info = f" ({provider.get('mode', 'N/A')})" if 'mode' in provider else ""
                    self.stdout.write(
                        self.style.SUCCESS(f'    - {provider["name"]}{mode_info}: {", ".join(provider["currencies"])}')
                    )
            else:
                self.stdout.write(
            ...
            self.stdout.write(
                self.style.ERROR(f'  ❌ Payment service error: {str(e)}')
            )
